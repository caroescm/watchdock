...
- [x] **Found and fixed a real gap while writing docs**: `action.yml` was still wired to the old M1 placeholder script (just prints the diff), not the real `still_detector` pipeline — meaning the actual shipped Action had never run the real logic, only local `nat run` testing had. Fixed to install `still_detector` and invoke it via `nat run`. **Verified via a real triggered GitHub Actions run** (PR against `main`, not local faking): correctly discovered `README.md` as the only target, correctly detected PR origin as `agent` (this session's commits carry the required `Co-Authored-By: Claude` trailer — an honest, unplanned validation of that logic), and correctly returned "no drift" (right call, since the README itself was the file being added/updated). Also surfaced a real, severe latency finding: one NIM call took ~19 minutes in this run (worse than any benchmark case, worth flagging honestly rather than hiding).
...
- [x] **Latency/accuracy optimization saga** (unplanned, prompted by the real Action's ~23min runtime): disabled thinking mode → 20-100x faster but recall dropped 100%→71% (proved via direct 9-case before/after comparison, not assumed) → re-enabling thinking alone didn't restore it (71% again, different cases missed — real nondeterminism, not a clean thinking-mode effect) → added a 3x parallel ensemble (union findings across concurrent samples) → 86% recall, better speed than the original slow run → root-caused the last miss via raw-output inspection: model reasoned correctly but ran out of its 8000-token thinking budget mid-deliberation over an irrelevant classification label → raised budget to 24000 + tightened the prompt to stop that deliberation → **final: 100% recall, 100% precision, reproducibly diagnosed, not lucky**. Full story in `BENCHMARK.md`. Also added diff-filtering and target-level parallelization as real, accuracy-neutral speed wins along the way, plus a formal regression test for the ensemble merge logic.
