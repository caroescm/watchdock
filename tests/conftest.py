"""Shared duck-typed stand-ins for the PyGithub objects the pipeline touches.
No real API calls anywhere in the suite.

Use the factories: `make_pr(...)` and `make_repo(...)` return fresh fakes
per test; keyword arguments inject the failure a test wants to provoke.
"""
