...
| **Still (ours)** | Docs + agent-instruction files | LLM semantic reasoning via NeMo Agent Toolkit | Suggestion (human) or auto-commit (agent-authored), origin-aware | GitHub, every PR | **Yes** | **Yes** |
...
| Day 4 | Build the Still-Bench eval set (~20-25 cases, split per Section 11), implement the deterministic baseline + single-prompt variant, run all three approaches, compute metrics. Use results to tune prompts if false-positive rate is high. |
