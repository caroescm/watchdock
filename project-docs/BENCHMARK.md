...
| `nim_client.MAX_STREAM_ATTEMPTS = 4` (whole-call retry) | A run got a clean 200, streamed for 10 minutes, then the server returned "Internal server error" mid-stream. The client's built-in retries only cover the initial request, so nothing retried it; `extract_claims` has no ensemble to fall back on, so the run died. Later, with 2 attempts, a first-PR run on a fresh repo had the same `extract_claims` stream closed by the peer ("incomplete chunked read") on both attempts, ~4.7 and ~5.6 minutes in, and the run died with no summary posted. | The whole streamed call is retried from scratch (a partial stream cannot be resumed), with exponential backoff; raised from 2 to 4 (configurable as `nim_stream_attempts`). A failed extraction now still posts a "could not run" summary before the run fails. |
...
| Tolerant stale-line matching (`fixes.locate`) | A live run detected the right drift in `CLAUDE.md` and drafted the right fix, but the model quoted a numbered, bolded list item without its `1.` and `**`, and in earlier runs quoted only the stale sentence of that line. The verbatim whole-line match failed each time, so the fix was never committed or suggested. | A quote also matches a line whose text is the same once markdown decoration is ignored, or one whole sentence of a line when exactly one line in the file has it. The fix is applied as the smallest edit inside the real line, so its decoration survives. Mid-sentence fragments are still refused. |
