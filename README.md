...
Watchdock is a GitHub Action that catches semantic drift — in both human-facing docs (`README.md`, `/docs`) and AI-agent instruction files (`AGENTS.md`, `CLAUDE.md`, `.cursor/rules`) — on every pull request, in any language. When a code change makes a stated claim, convention, or instruction wrong, Watchdock flags the exact line and fixes it: a suggestion comment when a human authored the PR, or a direct commit when an AI coding agent did.
...
During development, a PR renamed the config file `.still.yml` to `.still-config.yml` in code. Watchdock detected that the README's description was now stale, recognized the PR as agent-authored, committed the one-line fix itself, and explained what it did (this ran on an earlier, differently-named copy of this repo, so the original PR isn't browsable here — the commit and comment text below are unedited):

> 🔧 **Watchdock — semantic staleness**: committed a fix to `README.md` on this branch.
>
> **Why:** The CONFIG_FILENAME changed from `.still.yml` to `.still-config.yml`, so Watchdock now reads the explicit target list from `.still-config.yml` instead. This line still reads syntactically valid text but now describes an outdated config filename, making it semantically stale.
>
> **Old:** … (or reads an explicit list from `.still.yml`)
> **New:** … (or reads an explicit list from `.watchdock.yml`)
...
Add `.github/workflows/watchdock.yml` to your repo:
...
Get a free NVIDIA API key at [build.nvidia.com](https://build.nvidia.com) (no credit card, ~1,000 free inference credits) and add it as a repo secret named `NVIDIA_API_KEY`. That's it — no other setup. To try it end to end, open a PR that changes something your `README.md` or `AGENTS.md` describes (rename a command, a flag, a config key) and watch Watchdock flag and fix the stale line.
...
Watchdock has no outputs — it delivers results directly as PR comments and commits rather than exposing them to downstream steps.
...
On every PR, Watchdock:

1. **Discovers targets** — auto-detects `AGENTS.md`, `CLAUDE.md`, `.cursor/rules`, `conventions.md`, `README.md`, `/docs/**`. A `.watchdock.yml` at the repo root can replace that list (`targets:`) or subtract from it (`ignore:`); both take glob patterns such as `docs/generated/*`
...
- **NVIDIA NeMo Agent Toolkit** (`nvidia-nat`) — the Watchdock pipeline is registered as a NAT workflow (`watchdock_detector/`), invoked via `nat run`, not a raw API wrapper.
...
- **Source layout:** `src/watchdock/` is the core package, host-independent: `pipeline.py` (the whole run), `github_api.py`, `targets.py`, `diff.py`, `claims.py`, `prompts.py`, `parsing.py`, `fixes.py`, `report.py`, `nim_client.py`, plus `models.py` (the frozen dataclasses and enums every stage shares), `env.py` (every environment variable, in one place) and `errors.py`. `watchdock_detector/` is the NAT workflow package, a thin adapter that depends on `watchdock`. `tests/` is the pytest suite (no network needed) and `benchmark/` the eval harness (see below).
...
Watchdock-Bench compares three approaches on a self-built eval set: a deterministic baseline (mimicking Evidoc), a single-prompt LLM call, and Watchdock's full pipeline (claim extraction → per-claim 3x parallel ensemble checks, unioned). Final result on all 9 cases run:

| Approach | Recall | Precision |
|---|---|---|
| Deterministic baseline | 57% | 1.00 |
| Single-prompt LLM | 71% | 1.00 |
| **Watchdock (full pipeline)** | **100%** | **1.00** |

On the 3 cases specifically designed to be unsolvable by a deterministic checker: baseline 0%, single-prompt 67%, Watchdock 100%. After the per-claim redesign, the highest-signal subset (those 3 hardest cases, the historical truncation-bug case, and 2 clean controls) was re-run against the live API: same 100% recall and precision, with faster per-case times.
...
```bash
git clone https://github.com/caroescm/watchdock.git
cd watchdock
python3 -m venv venv && source venv/bin/activate   # Python 3.11-3.13 (nvidia-nat does not support 3.14 yet)
pip install -e ".[dev]" -e watchdock_detector
```

The single `pip install` resolves both local packages together: `watchdock` (core logic plus the `dev` extra for pytest) and `watchdock_detector` (the NAT entry point, which depends on `watchdock`). The action itself installs with `-c constraints.txt`, which pins every transitive dependency; refresh it with `pip freeze --exclude-editable > constraints.txt` after a deliberate upgrade.
...
GITHUB_REPOSITORY=owner/repo nat run \
  --config_file /path/to/watchdock/watchdock_detector/src/watchdock_detector/configs/config.yml \
  --input "check PR #12"
```

Model, ensemble size, concurrency cap, timeout and the scanned checkout are all fields on the workflow in [`config.yml`](watchdock_detector/src/watchdock_detector/configs/config.yml); the defaults are listed there.

## Compatibility

This repo's own CI (`.github/workflows/watchdock.yml`) is wired to exercise the action end-to-end on `ubuntu-latest` on every PR — the runner it was tested on during development. It hasn't been run on `macos-latest` or `windows-latest`. Both `run:` steps in [`action.yml`](action.yml) declare `shell: bash` explicitly (composite steps don't inherit a default shell), so they should work on all three GitHub-hosted runners — Windows runners ship Git Bash — but the Python/`pip`/`nat` toolchain behavior on macOS and Windows hasn't been verified in CI. If you hit a runner-specific issue, please open one.
...
- No per-language AST parsing — Watchdock reasons from raw diff/file text, which is what makes it language-agnostic, but is less precise than a formal parser for very large diffs.
