"""Everything that talks to GitHub: locating the PR, reading its diff and
files, and deciding who authored it."""
import json
import logging
import re
...
def get_pr_context(pr_number: int | None = None):
    ...
    token = env.require("GITHUB_TOKEN", "Pass the workflow's token as the github_token input.")
    repo_name = env.require("GITHUB_REPOSITORY", "Set it to owner/repo of the PR to check.")

    if pr_number is None:
        event_path = env.optional("GITHUB_EVENT_PATH")
        if not event_path:
            raise ConfigError(
...
def get_diff(pr) -> list[DiffEntry]:
    """The PR's changed files with their patches.
    ...
    the list, with a placeholder rather than the string "None" in the prompt."""
    return [
        DiffEntry(filename=file.filename,
                  patch=file.patch if file.patch is not None else NO_PATCH_PLACEHOLDER)
        for file in pr.get_files()
    ]


def get_file_at(repo, path: str, ref: str) -> str | None:
    """The text of ``path`` at ``ref`` (a sha or branch), or None if it can't
    be fetched or isn't text. Used to anchor suggestions and commits to the
    PR head rather than to the merge commit the Action checks out."""
    try:
...
# matching anywhere in the message would let unrelated text ("shipped-by:
# the release team") flip a PR to agent.
...
def detect_pr_origin_from_data(commit_messages: list[str], author_login: str = "") -> Origin:
    ...
    human's."""
    if any(_AGENT_TRAILER.search(message or "") for message in commit_messages):
        return Origin.AGENT
